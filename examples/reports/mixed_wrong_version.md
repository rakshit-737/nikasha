...
this is not a report about real software. Same real ASan output as the genuine fixture, but the report claims the wrong
version (v1.1.0). Expected verdict: MIXED, with "trace matches v1.2.0" and a version question.
-->

# Heap overflow in libhdr util_copy_value() when parsing long headers

**Affected:** libhdr 1.1.0 · CWE-122

In libhdr 1.1.0, `util_copy_value()` in `src/util.c` copies a header value into a 64-byte heap
buffer without checking its length, so a long header value overflows the buffer. It is reached
from `hdrcat` via `hdr_parse_block()` and `hdr_parse_line()`.
