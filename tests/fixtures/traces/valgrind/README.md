...
# `valgrind` trace fixtures

Real, unedited tool output. Regenerate with `python scripts/capture_trace_fixtures.py --only valgrind`.
...
|---|---|---|
| `01-vulnlab-invalid-write-v1.2.0.txt` | vulnlab v1.2.0, gcc -O0, valgrind, 96-byte value | `cp -r /src/vulnlab/v1.2.0 /work/libhdr && cd /work/libhdr && make -s CC=gcc CFLAGS="-O0 -g" >/dev/null 2>&1 && valgrind ./build/hdrcat /poc/large.txt` |
| `02-vulnlab-invalid-write-v1.2.1.txt` | vulnlab v1.2.1, gcc -O0, valgrind, 96-byte value | `cp -r /src/vulnlab/v1.2.1 /work/libhdr && cd /work/libhdr && make -s CC=gcc CFLAGS="-O0 -g" >/dev/null 2>&1 && valgrind ./build/hdrcat /poc/large.txt` |
| `03-vulnlab-small-overflow-v1.2.0.txt` | vulnlab v1.2.0, 70-byte value, --track-origins | `cp -r /src/vulnlab/v1.2.0 /work/libhdr && cd /work/libhdr && make -s CC=gcc CFLAGS="-O0 -g" >/dev/null 2>&1 && valgrind --track-origins=yes --leak-check=full ./build/hdrcat /poc/small.txt` |
