...
def test_fragment_carries_css_and_no_js():
...
def test_css_cannot_close_the_style_element():
    fragment = evidence_cards.render(make_context(make_evidence()))
    assert fragment is not None
...
def test_card_id_is_the_evidence_id():
    html = render(make_evidence(id="feedfacecafe"))
    assert 'id="ev-feedfacecafe"' in html


def test_card_id_survives_a_hostile_evidence_id():
...
def test_grouped_by_check_group_in_the_context_order():
...
def test_group_heading_carries_the_label_and_the_count():
    html = render(
...
def test_unknown_group_still_gets_a_heading():
    html = render(make_evidence(group="brand_new_group"))
    assert "Brand New Group" in html


def test_strongest_evidence_comes_first_within_a_group():
    html = render(
...
def test_card_shows_outcome_check_id_and_summary():
...
def test_outcome_pill_uses_the_context_class():
...
def test_model_evidence_is_marked_as_never_decisive():
...
def test_missing_excerpt_renders_the_card_without_code():
...
def test_no_provider_at_all_renders_the_card_without_code():
    html = render(make_evidence(locations=(make_location(),)))
    assert "<pre" not in html


def test_excerpt_stored_on_the_location_is_used():
...
def test_lines_are_numbered_and_the_cited_lines_are_highlighted():
...
def test_reversed_line_range_still_highlights():
...
def test_the_highlight_accent_follows_the_outcome():
    html = render(
...
def test_syntax_highlighting_marks_c_tokens():
    html = render(
...
def test_a_pathless_location_does_not_raise():
    html = render(
        make_evidence(
...
def test_pygments_output_is_escaped():
...
def test_a_very_long_excerpt_is_cut():
...
def test_only_the_first_few_locations_are_shown():
...
def test_a_safe_permalink_becomes_a_link():
...
def test_a_missing_permalink_is_simply_absent():
    html = render(make_evidence(locations=(make_location(permalink=None),)))
    assert "<a " not in html


def test_a_hostile_permalink_is_dropped():
    for bad in (
        "javascript:alert(1)",
...
def test_details_are_collapsed_and_sorted():
...
def test_a_truncated_command_says_so():
    record = CommandRecord(
        argv=("git", "log"),
        exit_code=1,
        stdout_sha256="c" * 64,
...
def test_a_very_long_argv_is_cut():
    record = CommandRecord(
...
def test_hostile_input_is_inert_everywhere():
    record = CommandRecord(
        argv=("git", HOSTILE),
        exit_code=0,
...
def test_hostile_input_cannot_break_out_of_an_attribute():
...
def test_degenerate_values_do_not_raise():
    item = make_evidence(
        id="",
        check_id="",
        claim_ids=(),
...
def test_it_fits_in_a_page_with_one_script():
