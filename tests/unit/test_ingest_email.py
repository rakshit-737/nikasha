...
def _load(name: str, tmp_path: Path, **kwargs: object):
...
class TestFixtures:
    ...
    def test_sender_identity_is_never_stored(self, name, tmp_path):
    ...
    def test_multipart_prefers_plain_text_and_stores_attachments(self, tmp_path):
    ...
    def test_html_only_falls_back_to_the_html_intake(self, tmp_path):
    ...
    def test_format_patch_mail_becomes_a_patch_block(self, tmp_path):
    ...
    def test_hostile_message_is_tamed(self, tmp_path):
...
class TestLimits:
    def test_attachment_caps_apply(self, tmp_path):
        msg = EmailMessage()
        msg["Subject"] = "big"
    ...
    def test_part_bomb_is_capped(self, tmp_path, monkeypatch):
    ...
    def test_header_only_parts_are_never_stored(self, tmp_path):
    ...
    def test_deep_nesting_is_capped(self, tmp_path):
    ...
    def test_nesting_that_breaks_the_parser_is_refused_cleanly(self, tmp_path):
    ...
    def test_body_truncation_is_reported(self, tmp_path):
        data = b"Subject: long\n\n" + b"z" * 500
...
class TestShapes:
    def test_no_text_part_warns(self, tmp_path):
        msg = EmailMessage()
    ...
    def test_empty_and_garbage_input_do_not_crash(self, tmp_path):
    ...
    def test_crlf_and_lf_give_the_same_report(self, tmp_path):
    ...
    def test_same_bytes_same_json(self, tmp_path):
    ...
    def test_folded_subject_is_one_line(self, tmp_path):
        data = b"Subject: first line\n  second line\n\nbody\n"
        assert ingest_eml(data, run_dir=tmp_path).title == "first line second line"

    def test_unparsable_date_is_none(self, tmp_path):
    ...
    def test_text_attachment_with_filename_is_not_body(self, tmp_path):
        data = (
            b"Subject: s\nMIME-Version: 1.0\n"
    ...
    def test_default_run_dir_is_private_and_content_addressed(self, tmp_path, monkeypatch):
...
def test_is_format_patch(subject, text, expected):
...
def test_never_crashes_on_random_bytes(data, tmp_path):
