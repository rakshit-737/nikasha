...
def test_every_grammar_and_query_loads():
    for lang in Lang:
...
def test_non_ascii_names_and_invalid_bytes():
...
def test_over_the_size_cap_is_not_parsed():
    source = b"int x;\n" * (MAX_FILE_BYTES // 7 + 1)
...
def test_quadratic_error_recovery_is_cut_off_by_the_time_budget():
...
def test_deep_nesting_beyond_the_query_cursor_limit():
...
def test_many_calls_regression_for_point_accessor_crash():
...
def test_unknown_language_is_reported_not_raised():
...
def test_language_given_as_string_value():
