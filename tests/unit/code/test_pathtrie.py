...
def test_spec_example_build_root_prefix():
...
def test_vulnlab_reporter_path():
...
def test_repo_paths_are_normalized_and_deduplicated():
...
def test_add_after_resolve_resorts():
    trie = PathTrie(["b/x.c"])
...
def test_disambiguate_several_defining_files():
