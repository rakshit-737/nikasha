...
def _assert_map_is_exact(original: str, report) -> None:
...
class TestText:
    def test_crlf_and_controls_are_normalized(self):
...
class TestMarkdown:
    ...
    def test_blocks_and_spans(self):
...
class TestHtml:
    def test_text_pre_code_and_scripts(self):
        html = (
...
class TestRoles:
    ...
    def test_guess_role(self, content, lang, heading, role):
...
class TestLoad:
    ...
    def test_detect_format(self, text, name, fmt):
        assert detect_format(text, name) == fmt

    def test_eml_is_not_supported_yet(self):
        with pytest.raises(UnsupportedInputError):
            detect_format("x", "r.eml")

    def test_load_report_and_errors(self, tmp_path):
        path = tmp_path / "r.md"
