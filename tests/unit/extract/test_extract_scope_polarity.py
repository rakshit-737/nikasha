...
class TestSlopcheckFailureCases:
    def test_poc_quoted_code_is_the_reporters(self):
    ...
    def test_bare_line_is_not_bound_to_a_distant_path(self):
        text = "The file lib/vauth/ntlm.c is involved.\n\nThe crash is at line 245 of the parser."
        line = one(text, "line")
        assert line.path is None

    def test_path_missing_prefix_is_still_a_project_claim(self):
...
class TestPolarity:
    ...
    def test_negated_claims_are_supporting(self):
...
class TestScope:
    ...
    def test_classify_path(self, path, provenance):
        assert classify_path(path) == provenance

    def test_symbol_tied_to_product_name(self):
        c = one("A heap overflow in libhdr `hdr_decode_value()` leads to RCE.", "symbol")
        assert c.provenance == "project_attributed"

    def test_unattributed_symbol_is_unscoped(self):
    ...
    def test_version_of_other_product(self):
        found = {
...
class TestRoles:
    def test_title_and_lead_symbols_are_core(self):
        text = (
    ...
    def test_bug_words_make_core(self):
        text = (
            "# Report\n\nIntro text here.\n\nMore text.\n\n"
    ...
    def test_peripheral_kinds(self):
...
class TestRealisticReportFindings:
    ...
    def test_html_comments_are_not_report_content(self):
        text = (
